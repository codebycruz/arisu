...
function Window.new(
  ...
  id --[[@param id number]],
  width --[[@param width number]],
  height --[[@param height number]]
)
...
function WindowBuilder:build(
  eventLoop --[[@param eventLoop EventLoop]]
) ---@return Window
...
function EventLoop:run(
  callback --[[@param callback fun(event: Event, handler: EventHandler)]]
)
  ...
  local Handlers = {
    ...
    [x11.ButtonPress] = function(window)
      callback(
        { window = window, name = "mousePress", x = event.xbutton.x, y = event.xbutton.y, button = event.xbutton.button },
        handler
      )
    end,
    ...
    [x11.ButtonRelease] = function(window)
      callback(
        { window = window, name = "mouseRelease", x = event.xbutton.x, y = event.xbutton.y, button = event.xbutton.button },
        handler
      )
    end,
  }
